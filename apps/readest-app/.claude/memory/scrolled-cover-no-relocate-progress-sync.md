...
**Status 2026-09-08:** BOTH MERGED (readest#6130 as fff0a08ad, foliate#93 as 98b82a5); not yet verified on a mobile/desktop build. foliate: readest/foliate-js#93 (branch `fix/scrolled-collapsed-visible-range`, commit 6eb3bc1); readest: readest/readest#6130 (`chrox:fix/scrolled-cover-relocate-progress-sync`, commit 7b8f40eb7) pinning that commit. foliate#93 squash-MERGED as 98b82a5 and readest re-pinned to it in 2b78487de (same day), so no orphaned pin. Live-verified in Chrome with the local config's location/xpointer/progress deleted from IndexedDB (`AppFileSystem` > `files` > `Readest/Books/<hash>/config.json`) and the serwist SW unregistered: cover relocate at 3.6s, pull, jump to the remote position at 5.6s. NOTE: a tab that loads with NO dev server on 3001 is served by the serwist cache (old code) and looks alive.
