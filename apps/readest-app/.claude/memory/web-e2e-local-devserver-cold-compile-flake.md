...
**2026-09 update (#6031): the warm-dev-server recipe no longer suffices.** Even against a pre-warmed `next dev` on :3000, 11/13 annotation tests failed with `<nextjs-portal>` (the dev error overlay, raised by the app's `next-view-transitions` unhandled rejection) intercepting every click — `playwright.config.ts` documents this as the reason CI uses a production build. The reliable local reproduction of CI is now: `pnpm build-web`, then `CI=1 pnpm test:e2e:web` (CI env flips webServer to `pnpm start-web`, no server reuse). That ran 24/24 in 59s with zero retries. Dev-mode local e2e results are structurally untrustworthy for click-driven specs, warm or cold.
